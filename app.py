# 
# app.py — VERSÃO FINAL CONSOLIDADA
...
def formatar_data_br(data_obj):
    """Converte date object para formato DD/MM/AAAA"""
    if data_obj:
...
st.write(f"**Data:** {datetime.now().strftime('%d de %B de %Y')}")
...
with st.form(key="formulario_triagem", clear_on_submit=False):
    ...
    with col5:
        ...
        profissional_diagnostico_options = [
            "Neuropediatra",
            "Psiquiatra Infantil",
            "Psicólogo Clínico",
            "Médico Generalista",
        ...
        profissional_diagnostico_selected = st.selectbox("Quem firmou o diagnóstico? *",
            options=profissional_diagnostico_options,
            key="profissional_diagnostico_selectbox")
        
        profissional_diagnostico_final = profissional_diagnostico_selected
        if profissional_diagnostico_selected == "Outro (especificar)":
            profissional_diagnostico_outro = st.text_input("Especifique o profissional:",
                key="profissional_diagnostico_outro")
            if profissional_diagnostico_outro:
                profissional_diagnostico_final = f"Outro: {profissional_diagnostico_outro}"
        
    with col6:
        nivel_suporte = st.selectbox("Nível de Suporte *",
            options=[1, 2, 3],
            format_func=lambda x: NIVEL_SUPORTE_OPTIONS[x],
            key="nivel_suporte")
    ...
    historico_gestacao = st.text_area("Como foi a gestação?",
        key="historico_gestacao", height=100)
    
    col7, col8 = st.columns(2)
    with col7:
        tipo_parto_options = ["Parto Normal", "Cesárea Programada", "Cesárea de Urgência", "Outro"]
        tipo_parto = st.selectbox("Tipo de parto",
            options=tipo_parto_options, key="tipo_parto")
    with col8:
        idade_gestacional = st.slider("Idade gestacional (semanas)",
            min_value=20, max_value=42, value=40, key="idade_gestacional")
    ...
    with col9:
        marco_cabeca = st.text_input("Sustentou cabeça", key="marco_cabeca")
    with col10:
    ...
    with col19:
        convulsoes_eeg = st.text_area("Histórico de Convulsões / EEG",
